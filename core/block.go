...
import (
    ...
    "sync"
    "time"
    "blockchain-go/transactions"
)

// Block represents a block in the blockchain
type Block struct {
    Timestamp     int64                      // Records when block was created/mined
    Transactions  []*transactions.Transaction // stores Transactions 
...
func NewBlock(txs []*transactions.Transaction, prevBlockHash []byte, targetBits int) *Block {
    block := &Block{
        Timestamp:     time.Now().Unix(),
        Transactions:  txs,
        PrevBlockHash: prevBlockHash,
        Hash:          []byte{}, // it will be calculated later on
        Nonce:         0,
    }
    
    // Initialize the hash
    block.Hash = block.CalculateHash(targetBits)
    return block
}

// IsGenesisBlock checks if this block is a genesis block
func (b *Block) IsGenesisBlock() bool {
    b.mu.RLock() // uses read lock because it only prevBlockHash
    defer b.mu.RUnlock()
    return len(b.PrevBlockHash) == 0
}

// HashTransactions returns a hash of the transactions in the block
// merkle tree basically
...
func (b *Block) PrepareData(nonce int, targetBits int64) []byte {
    ...
    data := bytes.Join(
        [][]byte{
            ...
            IntToBytes(targetBits), // mining difficulty
...
func (b *Block) UpdateHash() error {
    ...
    b.Hash = b.CalculateHash(24) // Using default target bits
    return nil
}

// CalculateHash calculates and returns the hash of the block
func (b *Block) CalculateHash(targetBits int) []byte {
    ...
    data := b.PrepareData(b.Nonce, int64(targetBits))
